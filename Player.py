...
class Player:
    ...
    def __init__(self, is1P:bool):
        ...
        self.__x = 350
        self.bulletWeak_pressed_past = False
        self.bulletMiddle_pressed_past = False
        self.bulletStrong_pressed_past = False
    ...
    def Move(self, key=[], ai_input : int = -1, bullets : list[Bullet.Bullet] = []):
        ...
        if ai_input == -1:
            ...
            #key[(キーコード)]は押されていると1、押されていない時は0を返す
            #bool(1)はTrue、bool(0)はFalseを返すため、bool(key[(キーコード)])で整数型をbool型に変換できる
            #威力小の弾
            bulletWeak_pressed_now = bool(key[self.bulletWeak])
            if bulletWeak_pressed_now == True and self.bulletWeak_pressed_past == False:
            ...
            #威力中の弾
            bulletMiddle_pressed_now = bool(key[self.bulletMiddle])
            if bulletMiddle_pressed_now == True and self.bulletMiddle_pressed_past == False:
            ...
            #威力大の弾
            bulletStrong_pressed_now = bool(key[self.bulletStrong])
            if bulletStrong_pressed_now == True and self.bulletStrong_pressed_past == False:
        ...
        #AI操作時
            ...
            if ai_input == 2:
                ...
                self.bulletMiddle_pressed_past = False
                self.bulletStrong_pressed_past = False
            if ai_input == 3 and self.bulletWeak_pressed_past == False:
                #威力小の弾発射
                self.currentEnergy -= Bullet.WEAK_COST
                bullets.append(Bullet.Bullet(self.__x+25,self.y, Bullet.BULLET_WEAK, self.bulletDirection))
                self.bulletWeak_pressed_past = True
                self.bulletMiddle_pressed_past = False
                self.bulletStrong_pressed_past = False

            if ai_input == 4 and self.bulletMiddle_pressed_past == False:
                #威力中の弾発射
                self.currentEnergy -= Bullet.MIDDLE_COST
                bullets.append(Bullet.Bullet(self.__x+25,self.y, Bullet.BULLET_MIDDLE, self.bulletDirection))
                self.bulletWeak_pressed_past = False
                self.bulletMiddle_pressed_past = True
                self.bulletStrong_pressed_past = False
            
            if ai_input == 5 and self.bulletStrong_pressed_past == False:
                #威力大の弾発射
                self.currentEnergy -= Bullet.STRONG_COST
                bullets.append(Bullet.Bullet(self.__x+25,self.y,Bullet.BULLET_STRONG, self.bulletDirection))
                self.bulletWeak_pressed_past = False
                self.bulletMiddle_pressed_past = False
                self.bulletStrong_pressed_past = True
            
            #移動+弾1つ撃ち
            if ai_input == 6:
                #左移動+威力小の弾発射
                self.__x -= Gunpoint_Speed
                if self.bulletWeak_pressed_past == False:
                    self.currentEnergy -= Bullet.WEAK_COST
                    bullets.append(Bullet.Bullet(self.__x+25,self.y, Bullet.BULLET_WEAK, self.bulletDirection))
                    self.bulletWeak_pressed_past = True
                else:
            ...
            if ai_input == 7:
                #左移動+威力中の弾発射
                self.__x -= Gunpoint_Speed
                if self.bulletMiddle_pressed_past == False:
                    self.currentEnergy -= Bullet.MIDDLE_COST
                    bullets.append(Bullet.Bullet(self.__x+25,self.y, Bullet.BULLET_MIDDLE, self.bulletDirection))
                    self.bulletMiddle_pressed_past = True
                else:
            ...
            if ai_input == 8:
                #左移動+威力大の弾発射
                self.__x -= Gunpoint_Speed
                if self.bulletStrong_pressed_past == False:
                    self.currentEnergy -= Bullet.STRONG_COST
                    bullets.append(Bullet.Bullet(self.__x+25,self.y,Bullet.BULLET_STRONG, self.bulletDirection))
                    self.bulletStrong_pressed_past = True
                else:
